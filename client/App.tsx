...
function App() {
	...
	const addTweet = useCallback((tweetID: string, y: number, x: number) => {
		...
		(message: IMessageEvent) => {
			...
			switch (type) {
				...
				case 'NFT':
					...
					break;
				case 'tweet':
					const tweetIndex = tweets.findIndex((tweet) => tweet.key === itemKey);
					const tweet = tweets[tweetIndex];
					if (tweet) {
						setTweets([
							...tweets.slice(0, tweetIndex),
		...
		const onMessageEvent = (message: IMessageEvent) => {
			switch (message.key) {
				...
				case 'chat':
					...
					break;
				case 'gif':
					console.log('GIF message received');
					if (message.value) {
						addGif(message.value, message.gifKey);
					}
					break;
				case 'tweet':
					console.log('tweet message received');
					if (message.value && message.y && message.x) {
						addTweet(message.value, message.y, message.x);
					}
					break;
				case 'image':
					if (message.value) {
	...
	const actionHandler = (key: string, ...args: any[]) => {
		switch (key) {
			...
			case 'tweet':
				...
				const { x, y } = generateRandomXY(true, true);
				console.log("Tweet added at " + x + ", " + y);
				socket.emit('event', {
					key: 'tweet',
					value: id,
					y: y,
