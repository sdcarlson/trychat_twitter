...
interface IBoardProps {
	...
	updateTweets: (tweets: ITweet[]) => void;
}
				...
				{gifs.map((gif) => (
					...
					>
						<BoardObject
							...
							{...gif}
							onPin={() => {
								console.log('gif pinned in board.tsx');
								pinGif(gif.key);
							}}
							onUnpin={() => {
								console.log('gif unpinnend in board.tsx');
								unpinGif(gif.key);
							}}
						/>
					</CSSTransition>
			...
			<TransitionGroup>
				{tweets.map((tweet) => (
					<CSSTransition
						key={tweet.id}
						timeout={5000}
						classNames="tweet-transition"
						onEntered={() => {
							if (!tweet.isPinned) {
								const index = tweets.findIndex((_tweet) => _tweet.id === tweet.id);
								updateTweets([...tweets.slice(0, index), ...tweets.slice(index + 1)]);
							}
						}}
					>
						<BoardObject
							...
							{...tweet}
							onPin={() => {
								console.log('tweet pinned in board.tsx');
								pinTweet(tweet.id);
							}}
							onUnpin={() => {
								console.log('tweet unpinned in Board.tsx');
								unpinTweet(tweet.id);
							}}
						/>
					</CSSTransition>
